...
TERMINAL_PORT = 8000
WRAPPER_PORT  = 8001
WRAPPER_DIR   = "/tmp/agentenazare-wrapper"
...
<head>
  ...
  <title>AgenteNazaré — Economia Regional</title>
  ...
  <style>
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .logo-nazare {{ font-family:"Syne",sans-serif; font-weight:800; font-size:17px;
    ...
    .btn-drive:hover  {{ background:var(--green-glow); border-color:rgba(76,175,80,.5); }}

    .btn-backup {{ color:var(--green); background:var(--green-dim);
                   border-color:rgba(76,175,80,.25); }}
    .btn-backup:hover {{ background:rgba(76,175,80,.2); border-color:rgba(76,175,80,.5); }}
    ...
    #toast.ok    {{ background:rgba(76,175,80,.15); border-color:rgba(76,175,80,.35); color:var(--green); }}
    #toast.err   {{ background:rgba(224,112,112,.15); border-color:rgba(224,112,112,.35); color:var(--red);   }}
    #toast.info  {{ background:var(--green-dim);       border-color:rgba(76,175,80,.35);  color:var(--green);  }}
    ...
    #modal {{
      background: #0e1320; border: 1px solid rgba(76,175,80,.15);
    ...
    .backup-item:hover {{ background: rgba(76,175,80,.05); color: var(--ink); }}
    ...
    .modal-close:hover {{ background: rgba(76,175,80,.08); color: var(--green); }}
...
<body>
  ...
  <div id="topbar">
    <div class="logo">
      <span class="logo-nazare">Agente</span><span class="logo-ai">Nazaré</span>
      <span class="logo-tag">v1.0</span>
      <span class="logo-brand">· Economia Regional</span>
    </div>
  ...
  <div id="footer">
    <span class="footer-brand">📈 AgenteNazaré</span>
    <span class="footer-sep"></span>
    <span style="color:var(--ink-muted);font-size:10.5px;">Automação Rigorosa em Economia</span>
    ...
    <div id="provider-modal">
      ...
      <div id="prov-step2" style="display:none;">
        ...
        <input id="prov-key-input" type="password"
          ...
          onfocus="this.style.borderColor='rgba(76,175,80,.4)'"
    ...
    function connectProvider() {{
      ...
      grid.innerHTML = PROVIDERS.map(p => `
        <button onclick="selectProvider('${{p.id}}')" style="
          ...
          onmouseover="this.style.background='var(--green-dim)';this.style.color='var(--green)';this.style.borderColor='rgba(76,175,80,.35)'"
...
def start_wrapper_server():
    _nazare_drive = "/content/drive/My Drive/AgenteNazaré"
    _base = _nazare_drive if os.path.isdir(_nazare_drive) else _folder_path
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if p == "/api/apikey":
                ...
                try:
                    marker = f"# agentenazare-key-{provider}"
...
.ready-badge {
  ...
  background:rgba(76,175,80,0.10);border:2px solid rgba(76,175,80,0.35);
  border-radius:12px;animation:glow 2s ease-in-out infinite;
}
.ready-icon { font-size:28px; }
.ready-text { font-family:'DM Mono',monospace;font-size:18px;font-weight:600;color:#4caf50; }
</style>
<div class="ready-container">
  <div class="ready-badge">
    <span class="ready-icon">📈</span>
    <span class="ready-text">AgenteNazaré pronta!</span>
  </div>
</div>"""))
...
@keyframes pulse-glow {{
  0%,100% {{ box-shadow:0 0 20px rgba(76,175,80,0.3),0 4px 12px rgba(0,0,0,0.3); }}
  50%      {{ box-shadow:0 0 40px rgba(76,175,80,0.6),0 6px 20px rgba(0,0,0,0.4); }}
}}
...
.pa-launch:hover::before {{ left:100%; }}
.pa-launch:hover {{ transform:translateY(-4px) scale(1.02);filter:brightness(1.1); }}
.pa-launch:active {{ transform:translateY(-1px) scale(0.99); }}
...
.pa-launch .arrow {{ font-size:28px;transition:transform 0.2s ease; }}
.pa-launch:hover .arrow {{ transform:translateX(8px); }}
</style>
<div class="btn-container">
  <a href="{banner_url}" target="_blank" class="pa-launch">
    <span class="btn-icon">📈</span>
    <span class="btn-text">
      <span class="btn-main">ABRIR A AGENTENAZARÉ</span>
      <span class="btn-sub">Economia Regional e Matrizes</span>
    </span>
